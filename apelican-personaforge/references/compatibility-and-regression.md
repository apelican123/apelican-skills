...
| 私人入口认证 | Bearer；显式兼容 X-API-Key、X-MCP-Token、query token；双 token 轮换 | 认证矩阵逐项通过 |
...
| 新手可用性 | 每一步含操作、验证、预期结果、停止条件 | [quick-start.md](quick-start.md) 验证门 1–15 完整 |
| 多设备 | 纯 Markdown 技能包、云端 URL 复用、Tunnel 主机边界、第二设备只读回归 | [cross-device-use.md](cross-device-use.md) 清单通过 |

## 对抗性场景

发布前逐一尝试证明实现会失败：

1. 客户端只能用旧 `?token=`，但新模板只读 Bearer；
2. 客户端入口 `X-API-Key` 被透明转发到上游；
...
10. 第二设备缺少本机绝对路径、隐含插件或私人 MCP 后无法使用；
...
15. 生成 ZIP 后目录层级错误、文件缺失或哈希与审计时不一致。
...
4. 所有相对链接、纯 Markdown、脱敏和跨平台扫描通过；
